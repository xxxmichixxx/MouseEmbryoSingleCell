...
#generate center labels
#lab1 <- x %>% dplyr::group_by(colData(sce)$celltype) %>% dplyr::mutate(dim1 = mean(dim1), dim1 = mean(dim1))

ggplot(x,aes(x=dim1,y=dim2,col=colData(sce)$celltype)) + geom_point() + 
               facet_wrap(vars(colData(sce)$stage)) + theme_classic() #+
        #geom_text(data=lab1,aes(label=`colData(sce)$celltype`),col="black")
```
...
ggplot(summed.filtered,aes(x=Lineage,y=logFC_N)) + geom_boxplot(notch=FALSE,fill="#7bccc4") +theme_bw()
...
ggsave("../boxplot_neuronal_mesodermal_FC_Lineages1-5_not_cpm_filtered_v2_FoxA_Tbx23_corrected.svg",device = "svg",height=5,width=6)

ggplot(summed.filtered,aes(x=Lineage,y=logFC_E)) + geom_boxplot(notch=FALSE,fill="#feb24c") +theme_bw()
...
ggsave("../boxplot_endodermal_mesodermal_FC_Lineages1-5_not_cpm_filtered_v2_FoxA_Tbx23_corrected.svg",device = "svg",height=5,width=6)
